...
def mariadb_putfile():
    # 上传文件
    fileNeedTransfer = []
    fileNeedTransfer.append("mariadb-10.1.16-linux-x86_64.tar.gz")
...
def mariadb_deploy():
    with cd(env.remote_dir):
        ...
        # 做软链
        run("""ln -s ./mariadb-10.1.16-linux-x86_64 ./mariadb && echo '软链创建成功！' || echo '软链已经存在！' """)
...
def mariadb_clean():
    with cd(env.remote_dir):
        ...
        run("rm -rf mariadb-10.1.16-linux-x86_64 mariadb")
